...
#'  fit <- est_S_Plus_Plus_MethodB(X, A, Z, Y, T)
#'  fit
...
est_S_Plus_Plus_MethodB <- function(X, A, Z, Y, T) { # nolint
  ...
  res0 <- sum((1 - T) * A[, n_time_points] * Expect_A_X * Y) /
  ...
  data_long <- reshape2::melt(data_long,
                   id.vars = c(X_col_names, "T", "Y", A_col_names, "subj"),
                   variable.name = "Z")
  temp <- list()
  temp[[1]] <- g2[[1]]
  for (i in 2:n_time_points) {
  ...
  for (i in 1:n_time_points) {
    Z0s_pred[[i]] <- predict(models_Z_X[[i]],
                             newdata = data.frame(X, T = rep(0, n)))
  }
  # Estimate alpha of Z_i given T for 1 (intercept) and Xs
  ...
  res1 <- sum(T * A[, n_time_points] * Expect_A_X * Y) /
  ...
  g4 <- T * A[, n_time_points] * Expect_A_X * Y
  g5 <- T * A[, n_time_points] * Expect_A_X


  # Estimating expection of deriatives using sample averages
  partial_g4_alpha <- list()
  part1_partial <- T * A[, n_time_points] * Y
  for (i in 1:n_time_points) {
    dim_z <- dim(Z[[i]])[2]
    partial_vec <- matrix(part1_partial * (Expect_AZ_X[[i]] - Expect_A_X *
